    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data, data_partial, value_centroid, value_std_pc, shell_grid_gt, min_gt,
            ...
            if num_iter == config.step_per_update:
                ...
                base_model.zero_grad()
            logger.info(f'--------------- {idx} -----------------')
            logger.info('Training loss metric : %.4f' % (loss))
            logger.info('Training loss_chamfer : %.4f' % (loss_chamfer))
            logger.info('Training loss_mse : %.4f' % (loss_mse))


            epoch_loss.append(loss.item())
            epoch_loss_chamfer.append(loss_chamfer.item())
            epoch_loss_mse.append(loss_mse.item())
            n_itr = epoch * n_batches + idx
            if train_writer is not None:
                train_writer.add_scalar('train/itr loss', loss, n_itr)
                train_writer.add_scalar('train/itr loss_chamfer', loss_chamfer, n_itr)
                train_writer.add_scalar('train/itr loss_mse', loss_mse, n_itr)

            batch_time.update(time.time() - batch_start_time)
            batch_start_time = time.time()

        if train_writer is not None:
            train_writer.add_scalar('train/epoch loss', np.mean(epoch_loss), epoch)
            train_writer.add_scalar('train/epoch loss_chamfer', np.mean(epoch_loss_chamfer), epoch)
            train_writer.add_scalar('train/epoch loss_mse', np.mean(epoch_loss_mse), epoch)
...
def validate(base_model, criterion, test_dataloader, epoch, val_writer, args, config, config_SAP, logger=None):
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data, data_partial, value_centroid, value_std_pc, shell_grid_gt, min_gt,
            ...
            loss_chamfer = base_model.module.get_loss(point_r, gt)
            loss_mse = criterion(psr_grid, gt_psr)
            #loss_chamfer = ChamferDisL1(point_r, gt)
            # loss_chamfer, _ = chamfer_distance(point_r, gt)
            eval_step_dict['loss_chamfer'] = loss_chamfer.cpu()
            eval_step_dict['loss_mse'] = loss_mse.cpu()
            #print("validate_", model_id,": loss_chamfer = ", loss_chamfer.cpu()," loss_mse = ",loss_mse.cpu())
            for k, v in eval_step_dict.items():
                eval_list[k].append(v)
            #每个batch size的平均指标 loss_chamfer 和 loss_mse
        ...
        if val_writer is not None:
            val_writer.add_scalar('Valid/epoch loss', metric_loss_val, epoch)
            val_writer.add_scalar('Valid/epoch loss_chamfer', eval_dict['loss_chamfer'], epoch)
            val_writer.add_scalar('Valid/epoch loss_mse', eval_dict['loss_mse'], epoch)
    return metric_loss_val
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data, data_partial, value_centroid, value_std_pc, shell_grid_gt, min_gt,
            ...
            if dataset_name == 'crown':
                ...
                # 生成粗略点云 point_r，主要用于获取最大最小值min_depoint,max_depoint
                ...
                if args.use_crown:
                    ...
                    for k, v in eval_dict.items():
                        logger.info('test metric MSE : %.4f' % (eval_dict['psr_l2']))
                        logger.info('test metric L1 : %.4f' % (eval_dict['psr_l1']))
